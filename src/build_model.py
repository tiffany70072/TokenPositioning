...
def autoencoder_seq2seq(src_max_len, tgt_max_len, src_token_size, tgt_token_size, latent_dim=128, teacher_forcing_ratio=0.5):
	rd = RandomUniform(minval=-0.08, maxval=0.08, seed=None)

	encoder_inputs = Input(shape=(None,), name='encoder_inputs')
	#print('(Build model) encoder_inputs =', encoder_inputs.shape)
	embedding = Embedding(src_token_size, latent_dim, embeddings_initializer=rd, input_length=None, mask_zero=True, name='emb')
	...
	decoder_state_input_h = Input(shape=(latent_dim,), name='decoder_input_h')
	decoder_outputs, state_h = decoder_gru(embedding(decoder_inputs), initial_state=decoder_state_input_h)
	#print('(Build model) decoder_outputs =', decoder_outputs)
	decoder_outputs = decoder_dense(decoder_outputs)
	#print('(Build model) decoder_outputs =', decoder_outputs)
	decoder_model = Model([decoder_inputs] + [decoder_state_input_h], [decoder_outputs] + [state_h])
